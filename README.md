...
```csharp
builder.Services.AddSendgoAccount(Environment.GetEnvironmentVariable("SENDGO_AGENT_TOKEN")!);
// Sendgo.AccountClient를 생성자에 주입합니다.
```
...
```csharp
// AddSendgo로 등록한 SendgoClient를 주입받아 사용합니다.
await client.GetTemplateFoldersAsync("notice");
```

코어 1.5.0 이상이 필요합니다. 전체 메서드는 [코어 문서](https://github.com/send-go/dotnet#템플릿-폴더-150)를 참고하세요.
